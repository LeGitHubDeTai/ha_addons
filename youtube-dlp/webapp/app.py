...
DOWNLOAD_DIR = os.environ.get('YTDLP_DOWNLOAD_DIR', '/media/youtube-dlp')
CONFIG_DIR = os.environ.get('YTDLP_CONFIG_DIR', '/share/youtube-dlp')
...
def download():
    ...
    format_choice = request.form.get('format', 'bestvideo+bestaudio/best')
    if format_choice == 'best':
        ytdlp_opts['format'] = 'bestvideo+bestaudio/best'
    elif format_choice == 'mp4':
        ytdlp_opts['format'] = 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best'
    elif format_choice == 'audio':
        ytdlp_opts['format'] = 'bestaudio/best'
        ytdlp_opts['postprocessors'] = [{'key': 'FFmpegExtractAudio', 'preferredcodec': 'mp3', 'preferredquality': '192'}]
    ...
    def run_download():
        try:
            cmd = ['yt-dlp', '-J', '--no-warnings', '-o', ytdlp_opts['outtmpl'], '-f', ytdlp_opts['format'], url]
            if 'postprocessors' in ytdlp_opts:
                cmd.extend(['--postprocessor-args', 'ffmpeg'])
            logger.info(f"Lancement yt-dlp: {' '.join(cmd)}")
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=0)
            with download_lock:
            ...
            with download_lock:
                if video_id in active_downloads:
                    ...
                    active_downloads[video_id]['error'] = 'Timeout'
