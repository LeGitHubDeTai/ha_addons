...
#
# Ex�cut� par s6-overlay AVANT les scripts d'init LinuxServer
# (init-adduser, init-config, ...). Gr�ce � `with-contenv`, les
# variables export�es ici sont propag�es aux services suivants.
set -e

# Charge bashio (version embarqu�e dans l'image HA ou fallback standalone)
...
# Dossiers pratiques cr��s au premier d�marrage (ignor�s s'ils existent d�j�)
for _dir in /share/nzbget/downloads /share/nzbget/completed /share/nzbget/queue; do
  if [ ! -d "$_dir" ]; then
    mkdir -p "$_dir" || bashio::log.warning "Impossible de cr�er ${_dir}"
  fi
done

# Ajuste le propri�taire des dossiers cr��s par l'add-on (jamais la racine des montages)
